...
void compileAssignSt(void) {
  ...
  /* Left side: one or more identifiers (with optional indexes) separated by commas */
  eat(TK_IDENT);
  compileIndexes();
  ...
  /* Right side: one or more expressions separated by commas */
  compileExpression();
  while (lookAhead->tokenType == SB_COMMA) {
    eat(SB_COMMA);
    compileExpression();
...
void compileRepeatSt(void) {
  ...
  eat(KW_REPEAT);
  /* repeat-body: sequence of statements (at least one) */
  compileStatements();
  eat(KW_UNTIL);
  compileCondition();
