...
void compileConstant(void) {
  if (lookAhead->tokenType == SB_PLUS) {
    eat(SB_PLUS); compileConstant2();
  } else if (lookAhead->tokenType == SB_MINUS) {
    eat(SB_MINUS); compileConstant2();
...
void compileRepeatSt(void) {   // Cho phép nhiều lệnh giữa repeat và until
  assert("Parsing repeat statement ...");
...
void compileVariableList(void) {
  compileVariable();
  compileVariableList2();
}

void compileVariableList2(void) {
...
void compileExpressionList(void) {
  compileExpression();
  compileExpressionList2();
}

void compileExpressionList2(void) {
  if (lookAhead->tokenType == SB_COMMA) {
    eat(SB_COMMA);
    compileExpression();
    compileExpressionList2();
