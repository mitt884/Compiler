...
/* Skip all blank characters (space, tab, newline, ...) */
void skipBlank() {
  while (charCodes[currentChar] == CHAR_SPACE)
    readChar();
}

/* Skip comment of the form (* ... *) */
void skipComment() {
  int prevChar = 0;

  /* At this point, currentChar is the first character AFTER "(*" */
  while (1) {
    if (currentChar == EOF) {
    ...
    if (prevChar == '*' && currentChar == ')') {
      /* End of comment "*)" found, move to next character and return */
      readChar();
      return;
    }
