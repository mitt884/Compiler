...
void compileBlock(void) {
  ...
  if (lookAhead->tokenType == KW_CONST) {
    ...
    do {
      eat(TK_IDENT);
      // Kiểm tra định danh hằng số mới
...
ConstantValue* compileUnsignedConstant(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    // Kiểm tra hằng số đã khai báo và lấy giá trị
...
ConstantValue* compileConstant2(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    // Kiểm tra định danh hằng số nguyên
    obj = checkDeclaredConstant(currentToken->string);
    constValue = duplicateConstantValue(obj->constAttrs->value);
    break;
  default:
...
Type* compileType(void) {
  ...
  case TK_IDENT:
    eat(TK_IDENT);
    // LẤY KIỂU TỪ ĐỊNH DANH (Sửa lỗi Arr(10, ))
...
void compileFactor(void) {
  ...
  case TK_IDENT:
    ...
    switch (obj->kind) {
    case OBJ_CONSTANT: break;
    case OBJ_VARIABLE: compileIndexes(); break;
    case OBJ_PARAMETER: break;
    case OBJ_FUNCTION: compileArguments(); break;
    default: 
